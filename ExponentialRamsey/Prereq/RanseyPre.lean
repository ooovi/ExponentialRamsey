...
#print monochromatic_subsingleton
...
theorem MonochromaticBetween.image {C : TopEdgeLabelling V' K} {X Y : Finset V} {k : K} {f : V ↪ V'}
    (hXY : (C.pullback f).MonochromaticBetween X Y k) :
    C.MonochromaticBetween (X.map f) (Y.map f) k := by
