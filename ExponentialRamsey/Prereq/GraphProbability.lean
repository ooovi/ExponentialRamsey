...
theorem weightingAux_sum_between (H₁ H₂ : SimpleGraph V)
  ...
  swap
  · simp only [edgeFinset_inf', compl_edgeFinset_eq]
    intros a aa b bb c
    simp_all
    have ad : Disjoint a H₁.edgeFinset := sorry
    have bd : Disjoint b H₁.edgeFinset := sorry
    rw [← disjUnion_eq_union _ _ ad,← disjUnion_eq_union _ _ bd] at c
    exact disjUnion_inj_right c
...
theorem basic_ramsey_bound {k l n : ℕ} {p : ℝ} (hp : 0 < p) (hp' : p < 1)
  ...
  obtain ⟨m, hm⟩ := h C
  rw [Fin.exists_fin_two, Matrix.cons_val_zero, Matrix.cons_val_one, ← indepOn_monochromaticOf,
    ← cliqueOn_monochromaticOf, toEdgeLabelling_labelGraph] at hm
